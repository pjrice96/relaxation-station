...
class Register_User_Handler(BaseHandler):
    # Not autheticated
    def post(self):
        applog("INFO: Register User Called")
        resp_obj = Map()
        resp_obj.handler = "Register_User_Handler"
        in_obj = Map()
        in_obj.user_name = self.get_argument('user_name', "")
        ...
        if 'user_name' in in_obj and 'user_email' in in_obj and 'user_password' in in_obj:
            relaxdb = RelaxDB()
            relaxdb.open()
            if not relaxdb.is_user(in_obj.user_email):
                # if email isn't registered, register user name, email and password
                user_id = relaxdb.add_user(in_obj.user_name,in_obj.user_email,in_obj.user_password)
                resp_obj.call_result = "SUCCESS"
            else:
                resp_obj.call_result = "FAILURE - Email",email,"has been used before. You may want to recover the password"
                print("Email",email,"has been used before. You may want to recover the password")
            relaxdb.close()
        else:
            resp_obj.call_result = "FAILURE - Missing a fields ", str(in_obj)
...
"""
class Sign_In_Handler(BaseHandler):
    # Not autheticated
    def post(self):
    ...
    def set_current_user_id(self, user_id):
        if user_id:
            self.set_secure_cookie("user_id", tornado.escape.json_encode(user_id), expires_days = None)
        else:
            self.clear_cookie("user_id")

class Sign_Out_Handler(BaseHandler):
    @tornado.web.authenticated
    def post(self):
        applog("INFO: Sign Out Called")
        resp_obj = Map()
        resp_obj.handler = "Sign_Out_Handler"
        user_id = self.get_current_user()

        applog("SECURITY: Signing out user_id %s\n" % user_id)
        self.set_current_user_id(False)
        resp_obj.call_result = "SUCCESS"
        self.write(json.dumps(resp_obj))
        self.finish()
...
>>>>>>> 3f133e65533e6a3c4f60f69a58d8955aa3b19937
